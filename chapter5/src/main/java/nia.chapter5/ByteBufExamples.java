...
public class ByteBufExamples {
    ...
    public static void directBuffer() {
        ...
        //检查 ByteBuf 是否由数组支撑(堆缓存区)。如果不是，则这是一个直接缓冲区
    ...
    //分配和复制操作，以及伴随着对数组管理的需要，使得这个版本的实现效率低下而且笨拙。这里推荐使用5-4
     ...
     */
    //代码清单 5-6 访问数据
    //需要注意的是，使用那些需要一个索弓|值参数的方法（的其中）之一来访问数据既不会改变 reade rIndex 也不会改变 writerIndex。
    // 如果有需要，也可以通过调用 readerIndex  (index)或者 writerIndex  (index）来手动移动这两者。
     ...
     */
    //代码清单 5-9 使用 ByteBufProcessor 来寻找\r,该方法已被废弃
     ...
     * Listing 5.10 Slice a ByteBuf
     */
    //代码清单 5-10 对 ByteBuf 进行切片(共享的，不是独立的数据)
    ...
    public static void byteBufCopy() {
        ...
        ByteBuf buf = Unpooled.copiedBuffer("Netty in Action rocks!", utf8);
        //创建该 ByteBuf 从索引 0 开始到索引 15 结束的分段的**副本**
